...
public class CaseNode extends RexNodeBase {
    ...
    private void buildExpr(RexNode node){
        ...
        while (count < operands.size()-1){
            ...
            RexNodeBase conditionConverter = RexNodeConverter.getRexConstrains(inputs,condition,z3Context);
    ...
    private BoolExpr getOutEqualValue(RexNode value, BoolExpr condition){
        RexNodeBase valueConstrains = RexNodeConverter.getRexConstrains(inputs,value,z3Context);
        ...
        BoolExpr defaultExpr = z3Context.mkAnd(condition,outputEqualValue,nullValue);
        return defaultExpr;
    }
}

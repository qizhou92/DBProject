...
public class BoolPredicate extends RexNodeBase {
    ...
    private void buildExpr(RexNode node){
        ...
        switch (sqlKind){
            ...
            case AND: {
                ...
                this.output = getAndNodeSymoblicColumn(operands,inputs,assignConstraints,z3Context);
                break;
            }
            case OR: {
                List<RexNode> operands = callNode.getOperands();
                this.output = getOrNodeSymoblicColumn(operands,inputs,assignConstraints,z3Context);
                break;
            }
            /*
            case IN: {
    ...
    private void buildNotExpr(RexCall node){
        ...
        RexNodeBase converter = RexNodeConverter.getRexConstrains(inputs,operand,z3Context);
    ...
    static public SymbolicColumn getAndNodeSymoblicColumn(Collection<RexNode> operands,List<SymbolicColumn> inputs,List<BoolExpr> assignConstraints,Context z3Context){
    ...
    static public SymbolicColumn getOrNodeSymoblicColumn(Collection<RexNode> operands,List<SymbolicColumn> inputs,List<BoolExpr> assignConstraints,Context z3Context){
        List<BoolExpr> resultValues = new ArrayList<>();
        List<BoolExpr> nullValues = new ArrayList<>();
        List<BoolExpr> nullSymbol = new ArrayList<>();
        for(RexNode operand:operands){
    ...
    private void buildINExpr(RexCall node){
        ...
        RexNodeBase exprConverter = RexNodeConverter.getRexConstrains(inputs,operands.get(0),z3Context);
        ...
        for(int i=1;i<operands.size();i++){
            ...
            RexNodeBase valueConverter = RexNodeConverter.getRexConstrains(inputs,value,z3Context);
